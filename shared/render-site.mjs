...
<html lang="en"><head>
  ...
  <header><a class="brand" href="#top"><img src="${esc(assetUrl(site.logo || site.hero))}" alt="${esc(site.name)}"></a><nav><a href="#services">Services</a><a href="#process">How it works</a><a href="#areas">Service area</a><a class="nav-call" href="${phoneHref(site.phone)}">${esc(site.navCall || "Free estimate")}</a></nav></header>
  <main>
    <section class="hero" id="top" style="background-image:linear-gradient(90deg,rgba(10,12,14,.91),rgba(10,12,14,.68) 48%,rgba(10,12,14,.08)),url('${esc(assetUrl(site.hero))}')">
    ...
    <section class="shell contact"><div><p class="eyebrow accent">Ready to get started?</p><h2>${esc(site.ctaHeadline)}</h2><p>${esc(site.ctaCopy)}</p></div><div class="contact-card"><a class="phone" href="${phoneHref(site.phone)}">${esc(site.phone)}</a>${site.email ? `<a href="mailto:${esc(site.email)}">${esc(site.email)}</a>` : ""}<p>Serving ${esc((site.serviceAreas || [site.city]).join(", "))}.</p></div></section>
  </main>
